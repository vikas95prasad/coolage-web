...
const MenuSidebar: React.FC<{ menus: any[] }> = ({ menus }) => {
  return (
    <div className='fixed'>
              ...
              <div className='no-scrollbar sm:ml-0 -ml-3 sm:mr-0 -mr-3'>
                ...
                {menus.map((menu) => (
                  <div className='relative justify-center flex-none p-2 lg:p-0 lg:w-full align-center'>
                    <a className='border-b-2 lg:border-b-0 border-primary text-default font-bold hover:text-default group w-full lg:border-l-2 lg:flex items-center text-center lg:text-left p-2 text-sm font-medium focus:color-primary'>{menu.label}</a>
                  </div>))}
                </div>
              </div>
            </nav>
